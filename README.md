...
## AI（Apple Intelligence 対応）

- **対応端末では Apple Intelligence のオンデバイス基盤モデル（FoundationModels）が
  対戦相手として思考します。** モデル処理はすべて端末内で完結（オフライン）。
- Apple Intelligence が使えない端末・不可用・出力不正時は、本ゲーム専用の**決定的な
  強力探索 AI**（反復深化 α-β＋脅威ベース評価＋置換表＋静止探索）へ自動フォールバック。
  どの環境でも必ず対戦が成立します。重い探索はバックグラウンド実行。
